...
class ModelService:
    ...
    def invokeModel(self,path:str,bakoutput:str):
        ...
        result=subprocess.run([path+"Swat_Edit.exe"],cwd=path, capture_output=True, text=True)
        ...
        # Copy the contents of the file to the backup file
            shutil.copy2(path+"output.rch", path+bakoutput+".output.rch")
            shutil.copy2(path+"output.sub", path+bakoutput+".output.sub")
